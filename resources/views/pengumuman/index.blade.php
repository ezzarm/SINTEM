...
<style>
    ...
    .pg-toolbar { display: flex; align-items: center; justify-content: space-between; gap: 8px; flex-wrap: wrap; }
    ...
    .pg-dd-menu {
        ...
        border-radius: 6px; box-shadow: 0 4px 16px rgba(0,0,0,0.08); z-index: 100; overflow: hidden; padding: 4px;
    }
    ...
    /* ── Right: locked sidebar ── */
    .pg-side { width: 272px; min-width: 272px; flex-shrink: 0; overflow: hidden; padding: 16px 32px 16px 0; display: flex; flex-direction: column; gap: 12px; }

    /* ── Post card ── */
    .pg-post { display: flex; gap: 12px; padding: 16px; background: #fff; border: 1px solid #ebebf0; border-radius: 8px; transition: border-color 0.15s; flex-shrink: 0; }
    .pg-post:hover { border-color: #d4d0f0; }
    .pg-avatar { width: 34px; height: 34px; border-radius: 50%; background: #f4f4f8; flex-shrink: 0; display: flex; align-items: center; justify-content: center; }
    ...
    .pg-side-card { display: flex; gap: 9px; padding: 8px 14px; transition: background 0.12s; }
    ...
    .pg-see-more { display: flex; align-items: center; justify-content: center; padding: 8px 14px; font-size: 12px; font-weight: 700; color: #7c3aed; text-decoration: none; border-top: 1px solid #f0f0f5; transition: background 0.1s; }
    ...
    /* ══════════════════════════════════════════════
       BREAKPOINTS
       ▸ Tablet  768–1023px : hide sidebar, reduce toolbar/body padding
       ▸ Mobile  < 768px   : compact toolbar, full-width search
       ▸ XS      < 480px   : stack toolbar vertically
    ══════════════════════════════════════════════ */

    /* ── Tablet ── */
    @media (max-width: 1023px) {
        .pg-toolbar-wrap { padding: 12px 20px 10px; }
        .pg-main         { padding: 14px 20px 24px; }
    ...
    @media (max-width: 767px) {
        .pg-toolbar-wrap { padding: 10px 16px 8px; }
        .pg-toolbar      { flex-wrap: wrap; gap: 6px; }
        .pg-main         { padding: 12px 16px 20px; }
        .pg-search       { width: 100%; }
    ...
    @media (max-width: 479px) {
        ...
        .pg-toolbar-left { justify-content: flex-start; }
...
<div class="pg-wrap">
                ...
                <div class="pg-toolbar-left">
                    ...
                    <div class="pg-dd-custom" id="sortDropdown">
                        ...
                        <div class="pg-dd-menu" id="sortMenu">
                            <button type="button" class="pg-dd-option {{ $sort === 'terbaru' ? 'selected' : '' }}" onclick="selectSort('terbaru', 'Terbaru')">
                            ...
                            <button type="button" class="pg-dd-option {{ $sort === 'terlama' ? 'selected' : '' }}" onclick="selectSort('terlama', 'Terlama')">
    ...
    <div class="pg-columns">
            ...
            <div class="pg-post">
                ...
                <div class="pg-post-body">
                    ...
                    @foreach($files as $file)
                        ...
                        <div class="pg-image">
                            <img src="{{ $file->file_data ?: asset('storage/' . $file->file_path) }}"
                                 alt="{{ $file->file_name }}"
        ...
        {{-- ── Right: locked sidebar (hidden on tablet/mobile) ── --}}
        <aside class="pg-side">
            ...
            <div class="pg-widget">
                <div class="pg-widget-head">
                    ...
                    <span class="pg-widget-count" style="background:#dbeafe;color:#1d4ed8;">{{ $items->where('type', 'event')->count() }}</span>
                </div>
            ...
            <div class="pg-widget">
                <div class="pg-widget-head">
                    <svg width="13" height="13" fill="none" stroke="#92400e" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke-width="1.8"/><line x1="12" y1="8" x2="12" y2="12" stroke-width="2" stroke-linecap="round"/><line x1="12" y1="16" x2="12.01" y2="16" stroke-width="2.5" stroke-linecap="round"/></svg>
                    <span class="pg-widget-title">Lost & Found</span>
                    <span class="pg-widget-count" style="background:#fef3c7;color:#92400e;">{{ $items->where('type', 'lost_found')->count() }}</span>
                </div>
                <div class="pg-widget-list">
                    ...
                    <div class="pg-side-card">
                        ...
                        <div class="pg-side-info">
                            <div class="pg-side-name">{{ $item->title }}</div>
                            <div class="pg-side-meta">{{ !empty($item->lost_type) ? ($item->lost_type === 'found' ? 'Ditemukan' : 'Hilang') . ' · ' : '' }}{{ \Carbon\Carbon::parse($item->created_at)->diffForHumans() }}{{ !empty($item->found_at) ? ' · ' . $item->found_at : '' }}</div>
                        </div>
                    </div>
                    @empty
                ...
                <a href="{{ route('temuan.index') }}" class="pg-see-more">Lihat semua</a>
                @endif
            </div>
        </aside>
    </div>
